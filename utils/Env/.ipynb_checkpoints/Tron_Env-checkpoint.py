...
class Tron_Env:
    ...
    def resample_command(self,activate = True): # Only activate in walking, not stepping stone
        ...
        self.vel_cmd = (self.vel_cmd > 0.2).float()  # 70%概率前进，30%概率原地不动
        if not activate:
            self.vel_cmd[:] = 1

    def apply_disturbance(self,activate = True):
        agent_index = torch.randperm(self.agents_num,device = self.device)[:int(0.2*self.agents_num)] # 20% push
        num_agents = len(agent_index)
    ...
    def update_world(self, scaled_action):
        ...
        self.action = 0.3*scaled_action+0.7*self.prev_action
    ...
    def body_height_tracking_reward(self):
        ...
        below_min = (self.next_body_height - self.next_min_foot_z) < 0.75
    ...
    def body_ori_tracking_reward(self):
        reward_ori_track = -0.4 * self.next_body_ori.norm(dim=1, keepdim=True)

        reward_ori_track += -0.2 * self.next_L_foot_angle.view(-1,1).norm(dim=1, keepdim=True)
        reward_ori_track += -0.2 * self.next_R_foot_angle.view(-1,1).norm(dim=1, keepdim=True)
    ...
    def foot_air_time_reward(self):
        ...
        feet_air_time *= 2 *  (self.vel_cmd ==1).float()
