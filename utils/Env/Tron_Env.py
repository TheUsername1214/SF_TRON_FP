...
class Tron_Env:
    ...
    def get_current_observations(self):
        ...
        current_state = torch.concatenate(
            (self.joint_pos + current_noise[0],
             self.joint_vel*0.05 + current_noise[1],
             self.body_ori*0.25,
             self.angular_velocities*0.25 + current_noise[3],
             self.sine_clock,
             self.cosine_clock,
             self.action,
             self.vel_cmd,
    ...
    def get_next_observations(self):
        ...
        next_state = torch.concatenate((self.next_joint_pos + next_noise[0],
                                        self.next_joint_vel*0.05 + next_noise[1],
                                        self.next_body_ori*0.25,
                                        self.next_angular_velocities*0.25 + next_noise[3],
                                        self.sine_clock,
                                        self.cosine_clock,
                                        self.action,
                                        self.vel_cmd,
    ...
    def single_support_reward(self):
        ...
        is_stance_L = phase_L<0.7
        is_stance_R = phase_R<0.7
        ...
        """New"""
        walking_phase_reward += 0.3*(~(is_stance_L^self.next_L_foot_contact_situation)).float()-0.5 # same, then plus
    ...
    def stand_still(self):
        stand_still_reward = -1*self.joint_pos[:,2:].abs().mean(dim=-1,keepdim=True)
        stand_still_reward += -0.3*self.joint_vel.abs().mean(dim=-1,keepdim=True)
        stand_still_reward += -1 * self.next_body_ori[:,:2].norm(dim=1, keepdim=True)
