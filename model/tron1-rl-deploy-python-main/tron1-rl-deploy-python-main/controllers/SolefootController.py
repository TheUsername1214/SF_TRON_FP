...
class SolefootController:
    ...
    def initialize_onnx_models(self):
        ...
        self.policy_session = ort.InferenceSession(self.model_policy, sess_options=session_options,
                                                   providers=cpu_providers)
        self.policy_input_names = [self.policy_session.get_inputs()[i].name for i in
                                   range(self.policy_session.get_inputs().__len__())]
        self.policy_output_names = [self.policy_session.get_outputs()[i].name for i in
                                    range(self.policy_session.get_outputs().__len__())]
        self.policy_input_shapes = [self.policy_session.get_inputs()[i].shape for i in
                                    range(self.policy_session.get_inputs().__len__())]
        self.policy_output_shapes = [self.policy_session.get_outputs()[i].shape for i in
                                     range(self.policy_session.get_outputs().__len__())]
    ...
    def run(self):
        ...
        self.robot_cmd.Kd = [1.0 for _ in range(0, self.joint_num)]
    ...
    def compute_observation(self):
        ...
        if self.rl_type == "isaaclab":
            ...
            actions = self.swap_positions(actions)  # gym order 2 lab order

        sine_clock = np.sin(time.time()).reshape(1)
        cosine_clock = np.cos(time.time()).reshape(1)
        obs = np.concatenate([
            joint_pos_input,  # Scaled joint positions
            joint_velocities,  # Scaled joint velocities
            euler_angles,  # Scaled base orientation (Euler angles)
            base_ang_vel,  # Scaled base angular velocity
            sine_clock,
            cosine_clock,
            actions,  # Lab order
            self.cmd,
    ...
    def compute_actions(self):
        ...
        output,_ = self.policy_session.run(self.policy_output_names, inputs)

        # Flatten the output and store it as actions
        self.actions = np.array(output).flatten()  # Lab order
    ...
    def sensor_joy_callback(self, sensor_joy: datatypes.SensorJoy):
        ...
        linear_x = 1.0 if linear_x > 1.0 else 0.0
    ...
    def depth_image_callback(self, msg):
        ...
        try:
            ...
            # 更新深度图像数据
            self.depth_image = np.array(depth_array)

            self.depth_image_timestamp = msg.header.stamp.to_sec()
            self.depth_image_received = True

            np.save("origin_array",self.depth_image)
            self.depth_image = self.downsample_depth_image(self.depth_image)
            print(self.depth_image)
            np.save("filter_array",self.depth_image)
            # 可选：记录接收信息
            # rospy.loginfo_throttle(1.0, 
            #     f"深度图像接收: {msg.width}x{msg.height}, 时间戳: {self.depth_image_timestamp}")
    ...
    def downsample_depth_image(self, depth_image, target_shape=(11, 18), 
                            zero_threshold=0.01, neighborhood_radius=3):
        """
        降采样深度图像，并过滤零值噪声
        ...
        for i in range(h_target):
            for j in range(w_target):
                ...
                # 过滤噪声
                valid_pixels = region[region > zero_threshold]
                ...
                if len(valid_pixels) > 0:
                    ...
                    # 方法2：取中值（对噪声更鲁棒）
                    # downsampled[i, j] = np.median(valid_pixels)
                else:
